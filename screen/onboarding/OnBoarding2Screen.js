...
import {
  ...
  Keyboard,
  TouchableWithoutFeedback, Platform, NativeModules,
} from "react-native";
...
const OnBoarding2Screen = ({ navigation }) => {
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <KeyboardAvoidingView style={{ flex: 1, backgroundColor: "#fff", justifyContent: "space-between" }}
        ...
        <View style={{ alignItems: "center" }}>
          ...
          <View style={{ width: "100%", paddingHorizontal: 15, marginTop: 43 }}>
            <TextFieldOnBoarding text={nickname} NicknameFunction={NicknameFunction} />
          </View>
        </View>
        <CheckButtonRectangle text={"확인"} activate={NicknameValidCheck()}
                              disabled={!NicknameValidCheck()}
                              onPress={() => navigation.navigate('OnBoarding1Screen', {activate: 3, nickname: nickname.value})} />
      </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
  blueText: {
    color: CommonColor.main_blue, marginBottom: 30, marginTop: 105,
  },
});
